...
function mapinit()
{
	var mapOptions = {
          zoom: 10,
          center: new google.maps.LatLng(-36.853, 174.767),
          zoomControl: false,
		  panControl:false,
          scaleControl: true,
	...
	{ 
		...
		_infowin.close();
		_loca = e.latLng;
		if (!_editon) { _mapmark.setPosition(_loca); _mapmark.setVisible(true); }	
		_geocoder.geocode( { latLng:_loca }, function(responses)
		{ 
			if (responses && responses.length > 0){ _addr = responses[0].formatted_address; } else { _addr = ""; }
			updateobjaddr(_addr);
		});
		if (_strviewon) { _strviewser.getPanoramaByLocation(_loca, 30, showstrview); }
	});
	
	google.maps.event.addListener(_mapmark, 'dragend', function() {
		setobjcenter(this);
		var pos = _mapmark.getPosition();
	...
	google.maps.event.addListener(_strviewpano, 'position_changed', function() {
		var pos = _strviewpano.getPosition();
		_mapmark.setPosition(pos);
		_geocoder.geocode( { latLng:pos }, function(responses)
		{ 
			if (responses && responses.length > 0){ _addr = responses[0].formatted_address; } else { _addr = ""; }
			updateobjaddr(_addr);
	...
	for (i = 0; i < md.length; i++)
	{
		...
		switch(md[i][6])
		{
			...
			case "P":
				...
				obj = new google.maps.Polygon( { paths:pa, strokeColor:'#000000', strokeOpacity:1.0, strokeWeight:1, fillColor:"#ff0000", fillOpacity:0.5 } );
				break;
			case "L":
				co = md[i][7].split(";");
				for (j = 1; j < co.length; j++)
				...
				obj = new google.maps.Polyline( { path:pa, strokeColor:'#000000', strokeOpacity:1.0, strokeWeight:1 } );
				break;
			default:
				break;
		}
		...
		google.maps.event.addListener(obj, 'click', function(e) 
		{
			...
			deselectfavs();
			setobjcenter(this);
			_infowin.setContent(makeiwcontent(this, "ciw", 0));
			_mapmark.setVisible(false);
			if (this.prop6 != "M")
			{
...
function onwindowsize()
{
	...
	document.getElementById('mapdiv').style.height = (vh - 100) + "px";
	...
	document.getElementById('btn9').style.left = (vw - 50) + "px";
...
{
	_geocoder.geocode( { 'address': adr}, function(results, status) {
		if (status == google.maps.GeocoderStatus.OK) 
		{
			var loc = results[0].geometry.location;
			_map.setCenter(loc);
			if (_strviewon) { _strviewser.getPanoramaByLocation(loc, 30, showstrview); }
		}
		else 
		{
			updateobjaddr("Geocode was not successful.", "#dd0000");
		}
	});
}
...
function updateobjaddr(str, clr) 
{
	...
	fld.value = str;
	if (clr == null || clr == undefined || str == "") { fld.style.color = "#404040"; } else{ fld.style.color = clr; }
}
	...
	_geocoder.geocode({'latLng':pos}, function(results, status)
	{
		if (status == google.maps.GeocoderStatus.OK) 
		{
			if (results[0])
			{
				...
				_infowin.setContent(makeiwcontent(ob, "siw", 1));
...
{
	//_favobjects.length
	var a = ["", "Principal residence", "", "Principal residence (corrected)", "Another residence", "Another residence (corrected)", "", "Convenience store", "Supermarket", "Laundry/Dry cleaners", "Post office", "Bank/Credit Union", "Pharmacy/Drug store", "Clothing store", "Library", "Bookstore", "Video/DVD store", "Fast food Restaurant", "Coffee place", "Non-fast food restaurant", "Bus/Subway/Train stop", "Swimming pool". "Indor recreation/exercise", "Public open space", "Beach/Lake/River/Creek", "Bike/Hiking/Walking trails", "Basketball court", "Other playing fields/courts", "Public park", "School with recreation facilities", "Any school", "Your school", "Sport", "Cultural/Religious activities", "Visit people", "School travel (to)", "School travel (from)", "Neighbourhood boundary"];
	return a[_currpg];
}

function setplacecat()
{
	if (_currpg >= 1 && _currpg <= 5){ return "Residences"; }
	else if (_currpg >= 7 && _currpg <= 19){ return "Shopping and services"; }
	else if (_currpg == 20) { return "Transport"; }
	else if (_currpg >= 21  && _currpg <= 29) { return "Recreation and sport"; }
	else if (_currpg >= 30  && _currpg <= 31) { return "School"; }
	else if (_currpg >= 32  && _currpg <= 33) { return "Recreation and sport"; }
	else if (_currpg >= 34  && _currpg <= 36) { return "Social activities"; }
	else if (_currpg == 37) { return "Neighbourhood"; }
	else{ return "Custom"; }
}
...
function makeiwcontent(obj, src, typ)		//typ 0=favs , 1=finds 
{
	var cn;
	var ro = "";
	...
	if (_currpg > 0)
	{
		...
		{
			cn += "<br><br><b><a href='javascript:saveiwcontent(" + obj.prop1 + "," + so + ")' style='text-decoration:underline;'>Save</a></b>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;";
		}
...
function hidemap()			//external interface
{
	var i;
	var c = 0;
