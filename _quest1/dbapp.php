<?php header("Content-type: text/html; charset=utf-8");
	...
	if($pvarx != "")
	{
		//$database = "treksoft_saut";
		//$username="treksoft_trksft";
		$database = "beanztre_saut";
		...
		elseif ($pvarx[0] == "lay" && $pvarx[1] == "s")			//save layout
		{
			...
			if($result)
			{
				$pv4 = mysql_real_escape_string($pvarx[4]);
				if(mysql_num_rows($result) > 0)
				{
					$querySQL = "update apps set layouts = '" .$pv4. "' where appid = '" .$pvarx[2]. "' and docid = '" .$pvarx[3]. "'";
				}
				else
				{
					$querySQL = "insert into apps (appid, docid, layouts) values('" .$pvarx[2]. "', '" .$pvarx[3]. "', '" .$pv4. "')";
				}
				$result = @mysql_query($querySQL);
				if($result)
				{
